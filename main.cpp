...
void remapCL(Mat src, Mat dst, Mat map_x, Mat map_y){
	...
	cl_mem GPUMapXVector = clCreateBuffer(GPUContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * dst_proc.rows * dst_proc.cols, map_x.data, &err);
	cl_mem GPUMapYVector = clCreateBuffer(GPUContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * dst_proc.rows * dst_proc.cols, map_y.data, &err);
...
void remapCPU(Mat src, Mat dst, Mat map_x, Mat map_y){
	for (int j = 0; j < dst.rows; j++)
	{
		...
		{
			int src_x = map_x.at<float>(j, i);
			int src_y = map_y.at<float>(j, i);
			if (src_x >= src.cols || src_y >= src.rows) continue;
